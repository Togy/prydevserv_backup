...
void BattlegroundIC::DoAction(uint32 action, uint64 var)
{
    ...
    if (!plr || !m_GunshipA || !m_GunshipH)
        return;

    plr->CastSpell(plr,SPELL_PARACHUTE,true);
    plr->CastSpell(plr,SPELL_SLOW_FALL,true);
    plr->SetTransport(plr->GetTeamId() == TEAM_ALLIANCE ? m_GunshipA : m_GunshipH);
    ...
    plr->m_movementInfo.t_guid = (plr->GetTeamId() == TEAM_ALLIANCE ? m_GunshipA : m_GunshipH)->GetGUID();
...
void BattlegroundIC::_NodeOccupied(uint8 node,Team team)
{
    ...
    if (node == BG_IC_NODE_QUARRY)
    {
        ...
        CastSpellOnTeam(SPELL_QUARRY, team);
    }
    if (node == BG_IC_NODE_REFINERY)
    {
        ...
        CastSpellOnTeam(SPELL_OIL_REFINERY, team);
    }
