        ...
        inline uint32 Gain(Player *pl, Unit *u)
        {
            ...
            else
            {
                ...
                if (gain != 0 && u->GetTypeId() == TYPEID_UNIT && ((Creature*)u)->isElite())
                {
					...
					sLog->outString("Using Elite Kill Rate is: %f ",sWorld->getRate(RATE_XP_KILL_ELITE));
					if (u->GetMap() && u->GetMap()->IsDungeon()) {
                        //gain = uint32(gain * 2.75);
						sLog->outDebug("Standard Elite Kill XP=%f Configured Elite Kill XP=%f",gain*2.75,gain*(sWorld->getRate(RATE_XP_KILL_ELITE)*1.375));
						gain = uint32(gain * (sWorld->getRate(RATE_XP_KILL_ELITE)*1.375)); //1.375 is garnered by dividing 2.75 / 2 to get the ratio
					} else {
						sLog->outDebug("Standard Elite Kill XP=%f Configured Elite Kill XP=%f",gain*2,gain*sWorld->getRate(RATE_XP_KILL_ELITE));
						gain *= sWorld->getRate(RATE_XP_KILL_ELITE);
                        //gain *= 2;
					}
				}
