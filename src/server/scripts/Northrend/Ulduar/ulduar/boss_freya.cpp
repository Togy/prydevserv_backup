...
class boss_freya : public CreatureScript
{
    ...
    struct boss_freyaAI : public BossAI
    {
        ...
        void JustDied(Unit *victim)
        {
            ...
            if (Creature* Ironbranch = Unit::GetCreature(*me, pInstance ? pInstance->GetData64(DATA_ELDER_IRONBRANCH) : 0))
                Ironbranch->RemoveFromWorld();
            if (Creature* Stonebark = Unit::GetCreature(*me, pInstance ? pInstance->GetData64(DATA_ELDER_STONEBARK) : 0))
        ...
        void EnterCombat(Unit* pWho)
        {
            ...
            if (Creature* Brightleaf = Unit::GetCreature(*me, pInstance ? pInstance->GetData64(DATA_ELDER_BRIGHTLEAF) : 0))
...
class boss_elder_brightleaf : public CreatureScript
{
    ...
    struct boss_elder_brightleafAI : public ScriptedAI
    {
        ...
        void JustDied(Unit *victim)
        {
            ...
            if (Creature* Ironbranch = Unit::GetCreature(*me, m_pInstance ? m_pInstance->GetData64(DATA_ELDER_IRONBRANCH) : 0))
                if (Ironbranch->isAlive())
                    EldersCount++;
                
            if (Creature* Stonebark = Unit::GetCreature(*me, m_pInstance ? m_pInstance->GetData64(DATA_ELDER_STONEBARK) : 0))
                if (Stonebark->isAlive())
                    EldersCount++;
...
class boss_elder_ironbranch : public CreatureScript
{
    ...
    struct boss_elder_ironbranchAI : public ScriptedAI
    {
        ...
        void JustDied(Unit *victim)
        {
            ...
            if (Creature* Brightleaf = Unit::GetCreature(*me, m_pInstance ? m_pInstance->GetData64(DATA_ELDER_BRIGHTLEAF) : 0))
                if (Brightleaf->isAlive())
                    EldersCount++;
